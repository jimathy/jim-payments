...
RegisterServerEvent('jim-payments:Tickets:Give', function(data, biller, gang)
	...
	if biller then -- If this is found, it ISN'T a phone payment, so add money to society here
		if gang then
			if Config.Management then exports['Renewed-Banking']:addAccountMoney(tostring(biller.PlayerData.gang.name), data.amount - takecomm)
		...
		elseif not gang then
			if Config.Management then exports['Renewed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - takecomm)
...
RegisterServerEvent("jim-payments:server:PolCharge", function(citizen, price)
	...
	if price > 0 then
		if not Config.FineJobConfirmation then
			...
			if Config.Management then exports['Renewed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), (price - commission))
...
RegisterServerEvent("jim-payments:server:PolPopup", function(data)
	...
	if data.accept == true then
		...
		if Config.Management then exports['Renewed-Banking']:addAccountMoney(tostring(biller.PlayerData.job.name), data.amount - commission)
